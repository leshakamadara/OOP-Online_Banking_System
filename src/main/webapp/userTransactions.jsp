...
<div class="dashboard-content">
    <h1 class="welcome-text">Transactions</h1>
    ...
    <div class="payees-header">
        ...
        <div class="payees-actions">
            <form action="${pageContext.request.contextPath}/transaction" method="get" id="searchForm">
                <div class="search-container">
                    <span class="material-icons search-icon">search</span>
                    <input type="text" name="searchTerm" class="search-input" placeholder="Search Transaction" value="${searchTerm}">
                </div>
            </form>
        </div>
    </div>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:forEach items="${transactions}" var="transaction" varStatus="status">
                            <tr>
                                <td>TRN0${transaction.transactionId}</td>
                                <td>${transaction.fromAccountNumber} - ${transaction.fromAccountName}</td>
                                <td>${transaction.toAccountNumber}</td>
                                <td><fmt:formatDate value="${transaction.transactionDate}" pattern="yyyy-MM-dd HH:mm" /></td>
                                <td>${transaction.bankName}</td>
                                <td><fmt:formatNumber value="${transaction.amount}" pattern="#,##0.00" /> LKR</td>
                                <td style="color: #25c16f;">${transaction.status}</td>
                            </tr>
                        </c:forEach>
                    </c:otherwise>
                </c:choose>
